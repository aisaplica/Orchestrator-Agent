...
.EXAMPLE
    .\svn-diff.ps1 "C:\Desarrollo\SVN\ScacsWeb\Ingenieros\src\trunk"
    .\svn-diff.ps1 "C:\Desarrollo\SVN\ScacsWeb\Ingenieros\src\trunk" "dotNet\Batch\BatchCirbe;dotNet\Web\AIS.PR.UI.Web"
#>
param(
...
if ($LASTEXITCODE -ne 0) {
    @{ error = "svn status fallo: $svnOutput" } | ConvertTo-Json
    exit 1
}
...
# Parsear cada linea de svn status
$changes = @()
foreach ($line in ($svnOutput -split "`n")) {
    ...
    # Aplicar filtro de scope si se proporciono
