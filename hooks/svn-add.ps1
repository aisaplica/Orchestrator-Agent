...
.PARAMETER Files
    ...
    Si se omite → detecta automaticamente todos los ficheros sin versionar (status ?)
    dentro del workspace, excluyendo bin/obj/.vs.

.EXAMPLE
    .\svn-add.ps1 "C:\Desarrollo\SVN\ScacsWeb\Ingenieros\src\trunk"
    .\svn-add.ps1 "C:\Desarrollo\SVN\ScacsWeb\Ingenieros\src\trunk" -Files "dotNet\Batch\BatchCirbe\Helper.cs,dotNet\Batch\BatchCirbe\IHelper.cs"
#>
param(
...
if ($Files) {
    ...
    # Auto-detectar mediante svn status o busqueda directa
    $hasSvnCli = $false
...
if (Test-Path $tortoisePath) {
    ...
    @{
        ...
        added           = $added
        failed          = $failed
        note            = "Añadido via TortoiseSVN. Puede requerir confirmacion visual si algun fichero fallo."
    } | ConvertTo-Json -Depth 3
    exit 0
}
...
@{
    ...
    instructions    = @(
        "SVN CLI y TortoiseSVN no disponibles para añadir automaticamente.",
